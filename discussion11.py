...
def add_pets_from_json(filename, cur, conn):
    ...
    for i in json_data:
        ...
        cur.execute("INSERT INTO Patients (pet_id, name, species_id, age, cuteness, agressiveness) VALUES (?,?,?,?,?,?)", (species, i['name'], int(i['age']), i['species'], int(i['cuteness']), int([i]['aggressiveness'])))
    conn.commit()
...
def non_aggressive_pets(aggressiveness, cur, conn):
    tup = []
    cur.execute(f"SELECT name FROM Patients WHERE agressivenes <= '{aggressiveness}'")
    rows = cur.fetchall()
    for row in rows:
        tup.append(row[0])
    return tup 
